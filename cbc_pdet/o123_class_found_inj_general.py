...
class Found_injections:
    ...
    def make_folders(self, run):
        ...
        try:
            os.mkdir(f'{os.path.dirname(__file__)}/{run}')
        except OSError as e:
        ...
        if self.alpha_vary is not None:
            ...
            path = f'{os.path.dirname(__file__)}/{run}/alpha_vary'
            try:
                os.mkdir(f'{os.path.dirname(__file__)}/{run}/alpha_vary')
            except OSError as e:
                if e.errno != errno.EEXIST:
                    raise
        else:
            path = f'{os.path.dirname(__file__)}/{run}'
    ...
    def get_opt_params(self, run_fit, rescale_o3 = True):
        ...
        try:
            ...
            raise Exception('ERROR in self.get_opt_params: There are not such files because there is not a fit yet with these options.')
    ...
    def get_ini_values(self):
        ...
        try:
            ...
            raise Exception('ERROR in self.get_ini_values: Files not found. Please create .dat files with the initial param guesses for this fit.')
    ...
    def joint_MLE(self, run_dataset, run_fit, methods = 'Nelder-Mead', precision = 1e-2):
        ...
        path = f'{os.path.dirname(__file__)}/{run_dataset}/' + self.path
